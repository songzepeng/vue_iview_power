...
export default {
  ...
  created() {
    request.getpermissionData2(this.params).then(res => {
    ...
    ActorPowerOk() {
      ...
      this.userPower.forEach(ur => {
        if (!this.checkedPowerArr.includes(ur.permissionId)) {
          let p2 = request.deleteRolePower(ur.id);
          promiseArr.push(p2);
        }
      });
  ...
  watch: {
    user() {
      ...
      request.getPermissionRoleData(this.user.id).then(res => {
